...
class verifyEmail {
    ...
    */
	...
	public function check($email) {
        ...
        $this->smtpMsg = $this->_streamResponse();
		$code = $this->_streamCode( $this->smtpMsg );
        ...
        /***
        * 550 5.1.1 <rich@o3optika.hu>: Recipient address rejected: User unknown in virtual mailbox table
        * 
        * 450 4.7.1 Client host rejected: cannot find your hostname, [ip cím]
    ...
    /**
     * check up e-mails from array, the domain is must be valid
     * @param string $emails Email addresses
     * @return boolean True if the valid email also exist
     */
    public function check_array($emails,$domain) {
        $result = FALSE;
        // Attila
        $this->smtpMsg = '';
        ...
        $mxs = $this->getMXrecords(self::parse_email($emails[0]."@".$domain));
        $timeout = ceil($this->max_connection_timeout / count($mxs));
        foreach ($mxs as $host) {
            print " | mx: ".$host."\n";
        ...
        */
        ...
        foreach($emails as $email_user) {
            ...
            if (!self::validate($email_user."@".$domain)) {
                //$this->set_error("{$email} incorrect e-mail");
                //$this->edebug($this->ErrorInfo);
                //if ($this->exceptions) {
                //    throw new verifyEmailException($this->ErrorInfo);
                ...
                $tomb_vissza[$email_user] = 0;
                print $email_user."@".$domain." : hibás! \n";
                
            } else {
                
                $this->_streamQuery("MAIL FROM: <{$this->from}>");
                $this->_streamResponse();
                $this->_streamQuery("RCPT TO: <".$email_user."@".$domain.">");
                $this->smtpMsg = $this->_streamResponse();
                
                $code = $this->_streamCode( $this->smtpMsg );
                
                $tomb_vissza[$email_user] = code_vissza($code);
                print $email_user."@".$domain." : ".$this->smtpMsg."\n";
            }
            
            $z++;
            
            // 10 ellenőrzés után újra csatlakozunk, vagy ha üres a string, mert hiba van !!! ez volt a lényeg
            if( $z > 4 || $code != 250 || empty($this->smtpMsg) ) {
                //$this->_streamQuery("RSET");
                //print $this->_streamResponse()."\n";
                
                print "Újracsatlakozás ";
                //print $this->_streamResponse()."\n";
                $this->_streamQuery("QUIT");
                fclose($this->stream);
...
function code_vissza($code) {
    ...
    */
    ...
    $jo_kod = array(250,450,451,452,552);
