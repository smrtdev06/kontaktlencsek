...
<!--
<tr><td>Dátum-ig</td><td><input type="text" id="datumig" name="datumig"  value="{$datumig}"></td></tr>
-->
<tr><td>Családnév </td>
...
<td align="right"><select size=1 name="oldal">
  ...
  <option value="eles_szemuvegek" {if $oldal=='eles_szemuvegek'}selected{/if}>Szemuvegek.hu</option>
</select></td>
</tr>
<tr><td>Tranz.id </td>
...
<td align="right"><select size=1 name="fiz_mod">
  ...
  <!--<input type="text" name="fiz_mod" value="{$fiz_mod}">--></td></tr>
...
<tr {if $z%2==0}bgcolor="#dcdcdc"{/if}>
    <td><a onclick="javascript:document.getElementById('adatok_{$egy.rend_id}').style.display = 'block';">{$egy.rend_id}</a></td>
    <td>{$egy.rend_datum}</td>
    <td align="right">{$egy.osszeg|number_format:0:",":"."}</td>
...
<tr>
<td colspan="14">
{include file="admin/rendeles_adatok.tpl"}
</td>
</tr>
...
{/foreach}
<tr bgcolor="#ddffdd"><td colspan="2">&nbsp;</td><td colspan="2" style="color:#ddffdd;">{$osszeg_osszesen|number_format:0:",":"."} Ft</td><td colspan="10" align="right">Összesen: {$z-1} db</td></tr>
</table>
